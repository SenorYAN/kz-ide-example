...
define(['jquery', 'configurableComponent', 'lib/mustache', 'ui', 'baiduMapApi'], function($, Component, mustache) {
	...
	var baiduMap = function(component) {
		...
		this.map = new BMap.Map(component.$configEl.find("#map")[0], {
	...
	baiduMap.prototype.addrlist = function() {
		var val = $("#addr").val();
		var AddressList = $("#auto-show");
		var ls = new BMap.LocalSearch(this.map);
		ls.disableFirstResultSelection();
		if (val != "") {
			ls.search(val);
			ls.setSearchCompleteCallback(function(rs) {
				if (ls.getStatus() == BMAP_STATUS_SUCCESS) {
					console.log(rs.getPageIndex());
					AddressList.html("");
					for (var i = 0; i < rs.Oo.length; i++) {
						AddressList.append("<a style=\"text-decoration:none\" onclick=\"$('#addr').val(this.innerText);$('#auto-show').css('visibility', 'hidden')\">" + rs.getPoi(i).title + "</><br>");
					}
					AddressList.append("<a href=\"#\">下一页</a>");
					AddressList.css("visibility", "visible");
				} else {
					AddressList.css("visibility", "hidden");
				}
			});
		}
		else AddressList.css("visibility", "hidden");
	};
	function seletaddr(val) {
		$("#addr").val(val);
	}
	baiduMap.prototype.mapSearch = function() {
		...
		myGeo.getPoint(val, function(point) {
			if (point) {
				...
				var ls = new BMap.LocalSearch(that.map);
				ls.disableFirstResultSelection();
				ls.search(val);
				ls.setSearchCompleteCallback(function(rs) {
					if (ls.getStatus() == BMAP_STATUS_SUCCESS) {
						...
						if (poi) {
							...
							that.map.clearOverlays();
							that.map.addOverlay(new BMap.Marker(poi.point, {
								icon: that.myIcon
							}));
  ...
  <div id="map"></div>                                                                                                \
		...
		renderConfigurator: function() {
			...
			var that = this;
			this.$propertyPanel.html(mustache.render(this.config_edit, {
			...
			this.$configEl.delegate("#addr", "input", function(ev) {
				map.addrlist();
			});
			this.$configEl.delegate("#auto-show", "mouseleave", function(ev) {
				var AddressList = document.getElementById("auto-show");
